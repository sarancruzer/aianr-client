   ...
   searchBot(){
    ...
    if(this.searchreq != null){
      ...
      this._service.getsearchResponse(this.searchreq).subscribe(response => {
            ...
            this.chatLists.push({'machine':true,"value":res.response,"created_at":Date.now(),'question':this.searchreq,'reaction':''});
            this.searchreq = ""
            console.log("-------------------------------");
            console.log(session);
 ...
 err =>{
  ...
  favtoAsk(question){
   ...
   updateFavourites(question){
    ...
    this._favouriteService.updateFavourites(question,flag).subscribe(response => {
      let res = response.result.data;
